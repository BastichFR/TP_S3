...
void* my_malloc(size_t size)
{
    ...
    if (get_total(chunk) >= size8 + HSIZE) {
        size_t remaining_size = get_total(chunk) - size8;
        header* new_chunk = (header*)((char*)chunk + size8 + HSIZE);

        new_chunk->size = remaining_size - HSIZE;
        new_chunk->free = YES;
...
void my_free(void* ptr)
{
    ...
    if (ptr != NULL) {
        ...
        // Merge the nexts FREE.
        header* next = h->next;
        if (next != NULL && next->free == YES) {
            h->next = next->next;
            h->size += next->size;
            next = next->next;
        }
        
        // Merge the previous FREE.
        header* current = get_sentinel();
        while (current->next != prog_break && current->next != h) {
            current = current->next;
        }

        if (current != h && current->free == YES) {
            current->size += h->size;
            current->next = h->next;
        }


        // Remove the last (useless) free 
        header* last_non_free = NULL;
        current = get_sentinel();
        while (current != prog_break) {
            if (current->free == NO)
                last_non_free = current;
            current = current->next;
        }

        if (last_non_free != NULL && last_non_free->next != prog_break)
            last_non_free->next = prog_break;
    }
}
...
void* my_realloc(void* ptr, size_t size)
{
    ...
    if (h->free == YES && get_total(h) >= size){
        h->size = size;
        return ptr;
    }
